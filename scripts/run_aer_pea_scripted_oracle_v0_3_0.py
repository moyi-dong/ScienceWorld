#!/usr/bin/env python3
"""Run the candidate 0.3.0 oracle without modifying the frozen 0.2.0 runner."""
...
ORACLE_PATH = CASE_ROOT / "hidden/oracle_v0_3_0.py"
...
from scripts.run_aer_pea_scripted_oracle import _read_jsonl  # noqa: E402
...
def run_one(
    ...
    *,
    ...
    service = EpisodeService(
        world, variation, case_root, trajectory_path, operator_path, 1_000
    )
    oracle_module = _load_case_module("aer_pea_oracle_v0_3_0", ORACLE_PATH)
    evidence_module = _load_case_module(
        "aer_pea_evidence_v0_2_0", CASE_ROOT / "hidden/evidence.py"
    )
    grader_module = _load_case_module(
        "aer_pea_grader_v0_2_0", CASE_ROOT / "hidden/grader.py"
    )
    try:
    ...
    _safe_write_json(
        run_dir / "hidden_reproduction_events.json", hidden_reproduction
    )
    ...
    _safe_write_json(
        run_dir / "run_metadata.json",
        {
            ...
            "variation": variation,
            "case_root": case_root,
            "policy": "world_blind_adaptive_oracle_v0_3_0_candidate",
